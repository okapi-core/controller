#!/bin/sh
set -eu

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<SQL
DO
\$\$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'okapi_oscar_user') THEN
    CREATE USER okapi_oscar_user WITH PASSWORD '${OSCAR_DB_PASSWORD}';
  ELSE
    ALTER USER okapi_oscar_user WITH PASSWORD '${OSCAR_DB_PASSWORD}';
  END IF;
END
\$\$;

CREATE SCHEMA IF NOT EXISTS okapi_oscar;
ALTER SCHEMA okapi_oscar OWNER TO okapi_oscar_user_admin;
GRANT CREATE ON SCHEMA okapi_oscar TO okapi_oscar_user;
GRANT USAGE ON SCHEMA okapi_oscar TO okapi_oscar_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA okapi_oscar TO okapi_oscar_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA okapi_oscar GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO okapi_oscar_user;

DO
\$\$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'okapi_web_migration_user') THEN
    CREATE USER okapi_web_migration_user WITH PASSWORD '${OKAPI_WEB_DB_MIGRATION_PASSWORD}';
  ELSE
    ALTER USER okapi_web_migration_user WITH PASSWORD '${OKAPI_WEB_DB_MIGRATION_PASSWORD}';
  END IF;
END
\$\$;

DO
\$\$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'okapi_web_user') THEN
    CREATE USER okapi_web_user WITH PASSWORD '${OKAPI_WEB_DB_PASSWORD}';
  ELSE
    ALTER USER okapi_web_user WITH PASSWORD '${OKAPI_WEB_DB_PASSWORD}';
  END IF;
END
\$\$;

CREATE SCHEMA IF NOT EXISTS okapi_web AUTHORIZATION okapi_web_migration_user;
ALTER SCHEMA okapi_web OWNER TO okapi_web_migration_user;
GRANT CONNECT ON DATABASE okapi_oscar TO okapi_web_migration_user;
GRANT CONNECT ON DATABASE okapi_oscar TO okapi_web_user;
GRANT USAGE ON SCHEMA okapi_web TO okapi_web_user;
REVOKE CREATE ON SCHEMA okapi_web FROM PUBLIC;
REVOKE CREATE ON SCHEMA okapi_web FROM okapi_web_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA okapi_web TO okapi_web_user;
ALTER DEFAULT PRIVILEGES FOR ROLE okapi_web_migration_user IN SCHEMA okapi_web GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO okapi_web_user;
SQL
